...
<style>
    .city<?php echo $shortcode['id3']; ?> span {
        color: #555;
        font-weight: 400;
        width: auto;
    }

    .city<?php echo $shortcode['id3']; ?> span:hover {
        color: #000;
        font-weight: 700;
        cursor: pointer;
    }
...
<div class="blocGeoLocate" id="bloc<?php echo $shortcode['id1']; ?>">
    ...
    <select id="<?php echo $shortcode['id1']; ?>"
            name="<?php echo $shortcode['id1']; ?>" <?php echo $geoData['onChangeCountry']; ?> >
        <option value=""> -</option>
...
<div class="blocGeoLocate" id="bloc<?php echo $shortcode['id2']; ?>">
    ...
    <select id="<?php echo $shortcode['id2']; ?>" name="<?php echo $shortcode['id2']; ?>"
            style="display:none;" <?php echo $geoData['onChangeRegion']; ?> >
        <option value=""> -</option>
    </select>
</div>
<div class="blocGeoLocate" id="bloc<?php echo $shortcode['id3']; ?>">
    <label><?php _e( 'City', 'wpGeonames' ) ?></label>
    <input type="text" id="<?php echo $shortcode['id3']; ?>" name="<?php echo $shortcode['id3']; ?>"
           style="display:none;"/>
...
<script>
    ...
    function geoDataRegion() {
        jQuery.post('<?php echo admin_url( 'admin-ajax.php' ); ?>', {
            'action': 'geoDataRegion',
            'country': jQuery('#<?php echo $shortcode['id1']; ?> option:selected').val()
        }, function (data) {
    ...
    function geoDataCity() {
        jQuery('#<?php echo $shortcode['id3']; ?>').show().keyup(function () {
            ...
            wpgeoajx = jQuery.post('<?php echo admin_url( 'admin-ajax.php' ); ?>', {
                'action': 'geoDataCity',
                'country': jQuery('#<?php echo $shortcode['id1']; ?> option:selected').val(),
                'region': jQuery('#<?php echo $shortcode['id2']; ?> option:selected').val(),
                'city': jQuery('#<?php echo $shortcode['id3']; ?>').val(),
                'nbcity':<?php echo $shortcode['nbcity']; ?>}, function (data) {
    ...
    function wpGeonameCityMap(ci, lat, lon, out) {
        jQuery('#<?php echo $shortcode['id3']; ?>').val(ci);<?php if ( empty( $shortcode['map'] ) ) {
		echo 'return;';
	} ?>
