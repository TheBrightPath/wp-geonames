...
function wpGeonames_admin() {
	...
	if ( isset( $_POST['wpGeonamesAdd'] ) ) {
		$zip = '<p style="font-weight:700;color:#D54E21;">' . wpGeonames_addZip( $urlc, $_POST ) . '</p>';
	} else if ( isset( $_POST['wpGeonamesClear'] ) ) {
		$zip = '<p style="font-weight:700;color:#D54E21;">' . wpGeonames_clear() . '</p>';
	} else if ( isset( $_POST['wpGeonamesPostalAdd'] ) ) {
		$zip = '<p style="font-weight:700;color:#D54E21;">' . wpGeonames_postalAddZip( $urlp, $_POST ) . '</p>';
	} else if ( isset( $_POST['wpGeonamesPostalClear'] ) ) {
		$zip = '<p style="font-weight:700;color:#D54E21;">' . wpGeonames_postalClear() . '</p>';
	}
	$wpGeoList = get_option( 'wpGeonames_dataList' );
	if ( ! $wpGeoList || ! empty( $_GET['checkData'] ) ) {
		$wpGeoList = wpGeonames_update_options();
	}
	...
	if ( ! empty( $wpGeoList['date'] ) ) {
		...
		if ( time() - $old > 31536000 ) { // 1 year : 31536000 - ?>
            ...
            <div class="notice notice-warning is-dismissible">
                <p>
                    <strong><?php _e( 'Datas are very old. You should Clear this table and Add new datas.', 'wpGeonames' ); ?></strong>
                </p>
    ...
    <div class='wrap'>
        <h2 class="nav-tab-wrapper">
            <a href="options-general.php?page=wpGeonames-options" class="nav-tab<?php if ( empty( $_GET['geotab'] ) ) {
				echo ' nav-tab-active';
			} ?>"><?php _e( 'General', 'wpGeonames' ); ?></a>
            <a href="options-general.php?page=wpGeonames-options&geotab=check"
               class="nav-tab<?php if ( isset( $_GET['geotab'] ) && $_GET['geotab'] == 'check' ) {
				   echo ' nav-tab-active';
			   } ?>"><?php _e( 'Check Datas', 'wpGeonames' ); ?></a>
            <a href="options-general.php?page=wpGeonames-options&geotab=edit"
               class="nav-tab<?php if ( isset( $_GET['geotab'] ) && $_GET['geotab'] == 'edit' ) {
				   echo ' nav-tab-active';
			   } ?>"><?php _e( 'Edit Datas', 'wpGeonames' ); ?></a>
            <a href="options-general.php?page=wpGeonames-options&geotab=help"
               class="nav-tab<?php if ( isset( $_GET['geotab'] ) && $_GET['geotab'] == 'help' ) {
				   echo ' nav-tab-active';
			   } ?>"><?php _e( 'Help', 'wpGeonames' ); ?></a>
        </h2>
		<?php if ( ! empty( $_GET['geotab'] ) ) {
		if ( $_GET['geotab'] == 'check' ) {
			wpGeonames_admin_check();
		} else if ( $_GET['geotab'] == 'edit' ) {
			wpGeonames_admin_edit();
		} else if ( $_GET['geotab'] == 'help' ) {
			wpGeonames_admin_help();
		} ?>
    ...
    <link rel="stylesheet" href="<?php echo plugins_url(); ?>/wp-geonames/sumoselect/sumoselect.css" type="text/css"
          media="all"/>
    ...
    <div id="wpGeonamesAddStatus"><img id="wpGeonameAddImg"
	...
	$cc = '';
	if ( $wpGeoList ) {
		foreach ( $wpGeoList as $k => $v ) {
			if ( strlen( $k ) == 2 ) {
				$cc .= $k . ' (<span style="color:#D54E21;">' . $v . '</span>)&nbsp;&nbsp;';
			}
		}
	}
        ...
        <table class="form-table">
            ...
            <tr valign="top">
                ...
                <td style="width:250px;">
                    <input type="checkbox" name="wpGeo0" value="1" checked disabled/><span
                            style="color:#bb2;"><?php _e( 'ID', 'wpGeonames' ); ?></span><br>
                    <input type="checkbox" name="wpGeo1" value="1" checked disabled/><span
                            style="color:#bb2;"><?php _e( 'Name', 'wpGeonames' ); ?></span><br>
                    ...
                    <input type="checkbox" name="wpGeo4" value="1" checked disabled/><span
                            style="color:#bb2;"><?php _e( 'Latitude', 'wpGeonames' ); ?></span><br>
                    <input type="checkbox" name="wpGeo5" value="1" checked disabled/><span
                            style="color:#bb2;"><?php _e( 'Longitude', 'wpGeonames' ); ?></span><br>
                    <input type="checkbox" name="wpGeo6" value="1" checked disabled/><span
                            style="color:#bb2;"><?php _e( 'Feature Class', 'wpGeonames' ); ?></span>
                </td>
                <td>
                    <input type="checkbox" name="wpGeo7" value="1" checked disabled/><span
                            style="color:#bb2;"><?php _e( 'Feature Code', 'wpGeonames' ); ?></span><br>
                    <input type="checkbox" name="wpGeo8" value="1" checked disabled/><span
                            style="color:#bb2;"><?php _e( 'Country Code', 'wpGeonames' ); ?></span><br>
                    <input type="checkbox" name="wpGeo9" value="1" checked disabled/><span
                            style="color:#bb2;"><?php _e( 'Country Code2', 'wpGeonames' ); ?></span><br>
                    <input type="checkbox" name="wpGeo10" value="1" checked disabled/><span
                            style="color:#bb2;"><?php _e( 'Admin1 Code', 'wpGeonames' ); ?></span><br>
                    <input type="checkbox" name="wpGeo11" value="1" checked disabled/><span
                            style="color:#bb2;"><?php _e( 'Admin2 Code', 'wpGeonames' ); ?></span><br>
                ...
                <td>
                    ...
                    <input type="checkbox" name="wpGeo16"
                           value="1"/><?php _e( 'Digital Elevation Model', 'wpGeonames' ); ?><br>
                    <input type="checkbox" name="wpGeo17" value="1"/><?php _e( 'Timezone', 'wpGeonames' ); ?><br>
                    <input type="checkbox" name="wpGeo18" value="1" checked disabled/><span
                            style="color:#bb2;"><?php _e( 'Modification Date', 'wpGeonames' ); ?></span>
                </td>
            </tr>
            <tr valign="top">
                <th scope="row"><label><?php _e( 'Choose type of data to insert', 'wpGeonames' ); ?></label></th>
                <td style="width:250px;">
                    <input type="checkbox" name="wpGeoA" value="1" checked disabled/><span
                            style="color:#bb2;"><?php _e( 'A : country, state, region', 'wpGeonames' ); ?></span><br>
                ...
                <td>
                    <input type="checkbox" name="wpGeoP" value="1" checked disabled/><span
                            style="color:#bb2;"><?php _e( 'P : city, village', 'wpGeonames' ); ?></span><br>
                    <input type="checkbox" name="wpGeoCity" value="1"
                           checked/><?php _e( 'P* : just city', 'wpGeonames' ); ?><br>
                    <input type="checkbox" name="wpGeoS"
                           value="1"/><?php _e( 'S : spot, building, farm', 'wpGeonames' ); ?>
                </td>
                <td>
                    <input type="checkbox" name="wpGeoT"
                           value="1"/><?php _e( 'T : mountain,hill,rock', 'wpGeonames' ); ?><br>
    ...
    <hr/>
    <div id="wpGeonamesPostalAddStatus"><img id="wpGeonamePostalAddImg"
                                             src="<?php echo plugins_url( 'wp-geonames/images/loading.gif' ); ?>"
                                             style="display:none;"/></div>
	...
	if ( $wpGeoList ) {
		foreach ( $wpGeoList as $k => $v ) {
			if ( strlen( $k ) == 3 ) {
				$cc .= substr( $k, 1 ) . ' (<span style="color:#D54E21;">' . $v . '</span>)&nbsp;&nbsp;';
			}
		}
	}
    ...
    <form method="post" id="wpGeonames_options3" name="wpGeonames_options3"
        ...
        <table class="form-table">
                ...
                <td>
					<?php if ( $page2 ) { ?>
                        <select name="wpGeonamesPostalAdd" id="wpGeonamesPostalAdd" multiple="multiple">
							<?php foreach ( $postal as $r2 ) {
								if ( strlen( $r2 ) == 6 && substr( $r2, 2 ) == '.zip' ) {
									echo '<option value="' . $r2 . '">' . $r2 . '</option>';
								}
							} ?>
            ...
            <tr valign="top">
                <th scope="row"><label><?php _e( 'Choose columns to insert', 'wpGeonames' ); ?></label></th>
                <td style="width:250px;">
                    <input type="checkbox" name="wpGeoPostal0" value="1" checked disabled/><span
                            style="color:#bb2;"><?php _e( 'Country Code', 'wpGeonames' ); ?></span><br>
                    <input type="checkbox" name="wpGeoPostal1" value="1" checked disabled/><span
                            style="color:#bb2;"><?php _e( 'Postal Code', 'wpGeonames' ); ?></span><br>
                    <input type="checkbox" name="wpGeoPostal2" value="1" checked disabled/><span
                            style="color:#bb2;"><?php _e( 'Name', 'wpGeonames' ); ?></span><br>
                    <input type="checkbox" name="wpGeoPostal3" value="1"/><?php _e( 'Admin1 Name', 'wpGeonames' ); ?>
                </td>
                <td>
                    <input type="checkbox" name="wpGeoPostal4" value="1"/><?php _e( 'Admin1 Code', 'wpGeonames' ); ?>
                    <br>
                    <input type="checkbox" name="wpGeoPostal5" value="1"/><?php _e( 'Admin2 Name', 'wpGeonames' ); ?>
                    <br>
                    <input type="checkbox" name="wpGeoPostal6" value="1"/><?php _e( 'Admin2 Code', 'wpGeonames' ); ?>
                    <br>
                    <input type="checkbox" name="wpGeoPostal7" value="1"/><?php _e( 'Admin3 Name', 'wpGeonames' ); ?>
                </td>
                <td>
                    <input type="checkbox" name="wpGeoPostal8" value="1"/><?php _e( 'Admin3 Code', 'wpGeonames' ); ?>
                    <br>
...
function wpGeonames_update_options() {
	...
	global $wpdb;
	$a     = array();
	$count = $wpdb->get_results( "SELECT COUNT(*) c, country_code FROM " . $wpdb->base_prefix . "geonames GROUP BY country_code" );  // benchmark allCountries : 7.633 sec
	foreach ( $count as $r ) {
		if ( $r->country_code ) {
			$a[ $r->country_code ] = $r->c;
		}
	}
	//
	$postal = $wpdb->get_results( "SELECT COUNT(*) c, country_code FROM " . $wpdb->base_prefix . "geonamesPostal GROUP BY country_code" );
	foreach ( $postal as $r ) {
		if ( $r->country_code ) {
			$a[ 'p' . $r->country_code ] = $r->c;
		}
	}
	//
...
function wpGeonames_admin_check() {
	...
	if ( $Gcountry ) {
		if ( isset( $_GET['cityid'] ) ) {
			$region = wpGeonames_get_region_by_country( $Gcountry );
		} else if ( isset( $_GET['postal'] ) ) {
			$outPostal = wpGeonames_get_postalCheck( $Gcountry, $Gpostal );
		}
	}
	?>
    <style>
    ...
    <script>
        ...
        function wpGeonameCityMap(ci, lat, lon) {
            ...
            var wpgeodata = new L.TileLayer('//{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                minZoom: 5,
                maxZoom: 14,
...
function wpGeonames_addZip( $url, $f, $t = 0 ) {
	...
	if ( empty( $_REQUEST['geoToka'] ) || ! wp_verify_nonce( $_REQUEST['geoToka'], 'geoToka' ) ) {
		return;
	}
	...
	if ( ! is_dir( $upl['basedir'] . '/zip/' ) ) {
		mkdir( $upl['basedir'] . '/zip/' );
	}
	if ( ! $t ) {
	...
	if ( $handle ) {
		...
		while ( ( $v = fgets( $handle, 8192 ) ) !== false ) {
			...
			if ( ! empty( $e[0] ) && isset( $e[18] ) && ! empty( $e[6] ) && strpos( $fe, $e[6] ) !== false ) {
				if ( 'P' == $e[6] && ! empty( $f['wpGeoCity'] ) && ! isset( $a[ $e[7] ] ) ) {
					continue;
				} // only city in Feature_code ($a)
				++ $c;
...
function wpGeonames_clear() {
	...
	if ( empty( $a ) ) {
		$wpdb->query( "ALTER TABLE " . $wpdb->base_prefix . "geonames ADD INDEX `index1` (`feature_class`,`feature_code`(3),`country_code`,`cc2`(2),`name`(3))" );
	}
	// *********************************************
	wpGeonames_update_options();
	if ( $q ) {
...
function wpGeonames_get_country( $postal = 0 ) {
	...
	if ( ! $postal ) {
		$q = $wpdb->get_results( "SELECT DISTINCT country_code FROM " . $wpdb->base_prefix . "geonames ORDER BY country_code" );
	} else {
		$q = $wpdb->get_results( "SELECT DISTINCT country_code FROM " . $wpdb->base_prefix . "geonamesPostal ORDER BY country_code" );
	}
	$result = array();
	foreach ( $q as $r ) {
...
function wpGeonames_sortCountry( $a, $b ) {
