...
const urlPostal = 'http://download.geonames.org/export/zip/';
	 ...
	 */
	static function getLocations( $args = array(), $output = OBJECT ) {
...
SQL;
	...
	protected static function createWhereFilterIn( $field, $values, $relation = 'AND', $delim = '"' ) {
	...
	static function getGeoNameClient() {
		return self::$geoNameClient ?: self::$geoNameClient = new GeoNamesClient( get_option( 'wpGeonames_username' ) ?: 'thebrightpath' );
	}
...
SQL;
	 ...
	 * @return array
	 */
	static public function &getFeatureClasses() {
	 ...
	 */
	static public function setFeatureClasses( &$featureClasses ) {
		self::$featureClasses = self::saveArray( 'featureClasses', $featureClasses );
	}

	static function &loadArray( $name ) {
		/** @noinspection PhpIncludeInspection */
	 ...
	 */
	static public function setFeatureCodes( &$featureCodes ) {
	 ...
	 * @return array
	 */
	static public function &getCountryCodes() {
	 ...
	 */
	static public function setCountryCodes( &$countryCodes ) {
	 ...
	 */
	static public function setTimeZones( &$timeZones ) {
		self::$timeZones = self::saveArray( 'timeZones', $timeZones );
	}

	function addNoCountries() {
	...
	function addLocations(
		...
		$saveField     = static function ( $fieldName ) use ( &$fieldNames ) {
		...
		if ( $this->loadFileIntoDb( $source, $this->tblLocations, $fields, $mode,
				static function ( $row ) use ( &$features ) {
	...
	function downloadZip( $name, $url, $filename = null, $force = false ) {

		$zipFile = ! empty( $filename )
			? strip_tags( stripslashes( filter_var( $filename, FILTER_SANITIZE_URL ) ) )
			: null;
		if ( ! preg_match( '@(?:^|[/\\\\])([^/\\\\]+?)(\.[^./\\\\]*)?$@', $zipFile ?: $url, $matches ) ) {
			return false;
		}
	...
	function loadFileIntoDb( $source, $table, $fields, $mode, $callback = null, $regexDelimiter = '@' ) {
		...
		$regex  = join( '\t', array_filter( array_map( function ( $field, $fieldName ) {
			return $field->regex
				? ( $field->save
		...
		while ( ( $line = fgets( $handle, 8192 ) ) !== false ) {
			...
			array_walk( $fields, function ( $field, $fieldName ) use ( &$matches, &$fieldValues, &$wpdb ) {
	...
	function update_options( $force = false ) {
		...
		if ( $force ) {
			...
			foreach (
				...
				if ( $page && preg_match_all( "@\shref=\"(?<url>[-.\w]+.zip)\">[^<]+</a>\s+(?<date>[-\d]*?)\s(?:[\d:]+)\s+(?<size>[\w.]+)\s+$@im", $page, $matches, PREG_SET_ORDER ) ) {
					array_walk( $matches, function ( $match ) use ( $name, &$options ) {
		...
		$old = self::$wpdb->get_var( "SELECT MAX(modification_date) FROM {$this->tblLocations} LIMIT 1" ); // benchmark allCountries : 5.172 sec
		if ( $old ) {
			$options['date'] = $old;
		}
	...
	function addCountries() {
		...
		return $this->loadFileIntoDb( $source, $this->tblCountries, $fields, 1,
			static function ( &$row ) {
				return self::$instance->checkCountry( $row['iso2'], $row['country'] );
			} );
	}

	function checkCountry( $country_code, $country_name = null ) {
		return $this->checkArray( 'countryCodes', $country_code, $country_name );
	}

	function checkArray( $name, $key, $value = null ) {
	 ...
	 * @return array
	 */
	static function &saveArray( $name, &$array ) {
		...
		file_put_contents( self::getEnums()->$name->file, <<<EOF
<?php
...
SQL;
	...
	static public function getEnums() {
		...
		return self::$enums ?: self::$enums = (object) [
	...
	static function Factory( $file = __FILE__ ) {
	...
	function addTimezones() {
		...
		return $this->loadFileIntoDb( $source, $this->tblTimeZones, $fields, 1,
			static function ( &$row ) {
				$row['city']    = str_replace( '_', ' ', $row['city'] );
				$row['caption'] = str_replace( '_', ' ', $row['time_zone_id'] );

				return self::$instance->checkTimeZone( $row['time_zone_id'], $row['country_code'] ) && self::$instance->checkCountry( $row['country_code'] );

			} );
	}

	function checkTimeZone( $time_zone_id, $country_code ) {
		return $this->checkArray( 'timeZones', $time_zone_id, $country_code );
	}

	function checkSearchParamsMinRequirements( ?array $params, ApiQueryStatus $apiResult ) {
		...
		if ( empty( $country ) ) {
			return $len >= $minAllCountries ? $params : null;
		}
		...
		if ( is_string( $country ) ) {
			return $len >= $minSingleCountry ? $params : null;
		}

		return $len >= $minMultipleCountries ? $params : null;

	}

	function checkSearchParams( ?array $params, ApiQueryStatus $apiResult ): ?array {
...
SQL;
		...
		$cached = array_sum( array_map( static function ( $cachedQuery ) {
		...
		if ( $cachedQuery === null ) {
			...
			$inQueryId     = join( ",", array_map( static function ( $cachedQuery ) {
...
SQL;
			...
			$cached = array_sum( array_map( static function ( $c ) {
				return $c->count;
			}, $countryRecordCount ) );
		}
	...
	static function getCachedQuery(
		...
		if ( is_array( $query ) ) {
			...
			$inCountryCode = $query['searchCountry'] ? " AND r.country_code IN ('" . join( "','", $query['searchCountry'] ) . "')" : '';
			$inQueryId     = " AND r.query_id IN (" . join( ",", array_map( static function ( $query ) {
...
SQL;
	 ...
	 */
	function cacheSearchResult( ApiQueryStatus $apiResult ) {
		...
		array_walk( $recordsToCache, static function ( wpGeonamesLocation $location ) {
		...
		array_walk( $apiResult->result,
			static function ( wpGeonamesLocation $location, $i )
			use ( &$cachedQuery, &$self ) {
	...
	function addAdminMenu() {
		...
		add_options_page( 'WP GeoNames Options', 'WP GeoNames', 'manage_options', 'wpGeonames-options', [
			$this,
			'adminMenu'
		] );
	}
	...
	function adminMenu() {
		...
		if ( ! empty( $wpGeoList['date'] ) ) {
			...
			if ( time() - $old > 31536000 ) { // 1 year : 31536000 - ?>
                ...
                <div class="notice notice-warning is-dismissible">
                    <p>
                        <strong><?php _e( 'Data is very old. You should Clear this table and Add new datas.', 'wpGeonames' ); ?></strong>
                    </p>
		...
		$geoTab = $_GET['geotab'] ?: false;
		?>

        <div class='wrap'>
            <h2 class="nav-tab-wrapper">
                <a href="options-general.php?page=wpGeonames-options"
                   class="nav-tab<?php if ( empty( $geoTab ) ) {
					   echo ' nav-tab-active';
				   } ?>"><?php _e( 'General', 'wpGeonames' ); ?></a>
                <a href="options-general.php?page=wpGeonames-options&geotab=check"
                   class="nav-tab<?php if ( $geoTab === 'check' ) {
					   echo ' nav-tab-active';
				   } ?>"><?php _e( 'Check Data', 'wpGeonames' ); ?></a>
                <a href="options-general.php?page=wpGeonames-options&geotab=edit"
                   class="nav-tab<?php if ( $geoTab === 'edit' ) {
					   echo ' nav-tab-active';
				   } ?>"><?php _e( 'Edit Data', 'wpGeonames' ); ?></a>
                <a href="options-general.php?page=wpGeonames-options&geotab=help"
                   class="nav-tab<?php if ( $geoTab === 'help' ) {
					   echo ' nav-tab-active';
				   } ?>"><?php _e( 'Help', 'wpGeonames' ); ?></a>
            </h2>
			<?php

			switch ( $_GET['geotab'] ?: null ) {
				case 'check':
					$this->admin_check();
					break;
				case 'edit':
	...
	function admin_check() {
        ...
        <h2><?php _e( 'Check your Geonames data', 'wpGeonames' ) ?> - <?php _e( 'Countries', 'wpGeonames' ); ?></h2>
        ...
        <div style="float:left;width:48%;overflow:hidden;">
            <label for="geoCheckCountry"><?php _e( 'Country', 'wpGeonames' ) ?></label><br/>
            <select id="geoCheckCountry" name="geoCheckCountry"
				...
				<?php foreach ( $country as $r ) {
					echo '<option value="' . $r->country_code . '" ' . ( ( $Gcountry == $r->country_code ) ? 'selected' : '' ) . '>' . $r->name . '</option>';
				} ?>

            </select>
        </div>
        <div style="float:left;width:48%;overflow:hidden;">
            <label for="geoCheckRegion"><?php _e( 'Region', 'wpGeonames' ) ?></label><br/>
            <select id="geoCheckRegion" name="geoCheckRegion" <?php if ( empty( $region ) ) {
				...
				<?php if ( ! empty( $region ) ) {
					foreach ( $region as $r ) {
						echo '<option value="' . $r->admin1_code . '" ' . ( ( $Gregion == $r->admin1_code ) ? 'selected' : '' ) . '>' . $r->name . '</option>';
					}
				} ?>
        ...
        <div style="float:left;width:48%;overflow:hidden;">
            <label for="geoCheckCity"><?php _e( 'City', 'wpGeonames' ) ?></label><br/>
            <input type="text" id="geoCheckCity" name="geoCheckCity"
                   onkeyup="wpGeonameListCity(this.value,'<?php echo $Gcountry; ?>','<?php echo $Gregion; ?>');" <?php if ( ! $Gregion ) {
				echo 'style="display:none;"';
			} ?> />
        ...
        <hr/>
        <h2><?php _e( 'Check your Geonames datas', 'wpGeonames' ) ?>
            - <?php _e( 'Postal codes', 'wpGeonames' ); ?></h2>
        <form name="geoCheckPostal" action="" method="GET">
            <input type="hidden" name="page" value="wpGeonames-options"/>
            <input type="hidden" name="geotab" value="check"/>
            <input type="hidden" name="geoToka" value="<?php echo $geoToka; ?>"/>
            <div style="float:left;width:48%;overflow:hidden;">
                <label><?php _e( 'Country', 'wpGeonames' ) ?><br/>
                    <select name="country" id="wpGeonamesPostalCountry">
                        <option value=""> -</option>
						<?php foreach ( $postalCountry as $r ) {
							echo '<option value="' . $r->country_code . '" ' . ( ( $Gcountry == $r->country_code ) ? 'selected' : '' ) . '>' . $r->name . '</option>';
						} ?>
            ...
            <div style="float:left;width:48%;overflow:hidden;">
                <label><?php _e( 'Postal codes', 'wpGeonames' ); ?><br/>
	...
	function get_country( $postal = 0 ) {
		...
		if ( ! $postal ) {
			$q = $wpdb->get_results( "SELECT DISTINCT country_code FROM " . $this->tblLocations . " ORDER BY country_code" );
		} else {
			$q = $wpdb->get_results( "SELECT DISTINCT country_code FROM " . $this->tblPostCodes . " ORDER BY country_code" );
		}
		$result = array();
		foreach ( $q as $r ) {
	...
	function get_region_by_country( $iso = '' ) {
		//
		global $wpdb;
		$result = array();
		if ( $iso ) {
	...
	function regionCode2( $iso = 'ZZZ' ) {
		$a = ',BE,';

		return strpos( $a, $iso ) !== false;
	}

	function get_postalCheck( $iso, $postal ) {
	...
	function admin_edit() {
		...
		if ( ! empty( $_GET['geoid'] ) && ! empty( $_GET['geodata'] ) ) {
			...
			$wpdb->update( $wpdb->base_prefix . self::urlLocations, array( 'name' => $a ), array( 'geoname_id' => $id ) );
			...
			if ( ! empty( $q ) ) {
				foreach ( $q as $v ) {
					...
					$o .= '<input type="button" class="button-primary" value="' . __( 'Change', 'wpGeonames' ) . '" onClick="document.forms[\'geoEdit\'].elements[\'geodata\'].value=document.forms[\'geoEdit\'].elements[\'geadata' . $v->geoname_id . '\'].value;document.forms[\'geoEdit\'].elements[\'geoid\'].value=' . $v->geoname_id . ';document.forms[\'geoEdit\'].submit();" />';
					$o .= '</div>';
				}
			}
		}
        ...
        <form name="geoEdit" action="" method="GET">
            ...
            <div style="float:left;margin-right:20px;overflow:hidden;">
                <label><?php _e( 'Type of data', 'wpGeonames' ) ?><br/>
                    <select id="geoType" name="geoType">
                        <option value="region" <?php if ( $GgeoType == 'region' ) {
							echo 'selected';
						} ?>><?php _e( 'Region', 'wpGeonames' ) ?></option>
                        <option value="city" <?php if ( $GgeoType == 'city' ) {
							echo 'selected';
						} ?>><?php _e( 'City', 'wpGeonames' ) ?></option>
                    </select></label>
            </div>
            <div style="float:left;margin-right:20px;overflow:hidden;">
                <label><?php _e( 'Data', 'wpGeonames' ) ?><br/>
                    <input type="text" name="geoSearch" value="<?php if ( ! empty( $_GET['geoSearch'] ) ) {
						echo $_GET['geoSearch'];
					} ?>"/></label>
            </div>
            <div class="submit">
                <input type="submit" class="button-primary"
                       ...
                       value="<?php _e( 'Search', 'wpGeonames' ) ?>"/>
            </div>
	...
	function admin_general( $wpGeoList ) {
        ...
        <link rel="stylesheet" href="<?php echo plugins_url(); ?>/wp-geonames/sumoselect/sumoselect.css"
              type="text/css"
              media="all"/>
        <div class='icon32' id='icon-options-general'><br/></div>
        <div>
            <a style="float:right;margin:20px;" href="http://www.geonames.org/"><img
                        src="<?php echo plugins_url( 'wp-geonames/images/geonames.png' ); ?>" alt="GeoNames"
                        title="GeoNames"/></a>
        </div>
        ...
        <div id="wpGeonamesAddStatus"><img alt="loading" id="wpGeonameAddImg"
                                           src="<?php echo plugins_url( 'wp-geonames/images/loading.gif' ); ?>"
        ...
        <form method="post" id="wpGeonames_options1" name="wpGeonames_options1"
              action="options-general.php?page=wpGeonames-options&geoToka=<?php echo $geoToka; ?>">
            <table class="form-table" style="width: auto;">
                <tr style="vertical-align: top;">
                    <th scope="row"><label
                                for="wpGeonamesAdd"><?php _e( 'Add data to WordPress', 'wpGeonames' ); ?></label>
                    </th>
                    <td>
						<?php if ( ! empty( $wpGeoList['filenames']['countries'] ) || ! empty( $geoManual ) ) { ?>
						...
						<?php } else {
							echo '<span style="font-weight:700;color:#D54E21;">' . __( 'No connection available or issue with PHP file_get_contents(url)', 'wpGeonames' ) . '</span>';
						} ?>

                    </td>
                    <td><a href="https://en.wikipedia.org/wiki/ISO_3166-1"
                           target="_blank"><?php _e( 'Official Country List', 'wpGeonames' ); ?></a></td>
                    <td>
                    </td>
                </tr>
                <tr style="vertical-align: top;">
                    ...
                    <tr style="vertical-align: top;">
                        <td>
                            <div class="button-primary" style="width: 150px;" onclick="wpGeonames_addCountries(0);">
								<?php _e( 'Add', 'wpGeonames' ) ?></div>
                            <br>
							<?php _e( 'Skip existing', 'wpGeonames' ) ?>
                        </td>
                        <td>
                            <div class="button-primary" style="width: 150px;" onclick="wpGeonames_addCountries(1);">
								<?php _e( 'Add and Update', 'wpGeonames' ) ?></div>
                            <br>
							<?php _e( 'Only replace selected fields, keep others', 'wpGeonames' ) ?>
                        </td>
                        <td>
                            <div class="button-primary" style="width: 150px;" onclick="wpGeonames_addCountries(2);">
								<?php _e( 'Add and Replace', 'wpGeonames' ) ?></div>
                            <br>
							<?php _e( 'Replace all fields (clearing existing ones)', 'wpGeonames' ) ?>
                        </td>
                        <td>
                            <div class="button-primary" style="width: 150px;"
                                 onclick="wpGeonames_addCountries(-1);">
								<?php _e( 'Update', 'wpGeonames' ) ?></div>
                            <br>
							<?php _e( 'Only update selected fields of existing records', 'wpGeonames' ) ?>
                        </td>
                    </tr>
				<?php } ?>
            </table>
        ...
        <form method="post" name="wpGeonames_options2"
            ...
            <p class="submit">
                <input type="submit" class="button-primary"
                       value="<?php _e( 'Clear this table (TRUNCATE)', 'wpGeonames' ) ?>"/>
            </p>
        </form>
        <hr/>
		...
		echo '<p>' . __( 'Number of data in this database', 'wpGeonames' ) . ' : <span style="font-weight:700;color:#D54E21;">'
		     . $wpdb->get_var( "SELECT COUNT(*) FROM " . $wpdb->base_prefix . "geonamesPostal" )
		     . '</span><a style="margin-left:10px;" href="options-general.php?page=wpGeonames-options&checkData=1"><img alt="reload" src="' . plugins_url( 'wp-geonames/images/reload.png' ) . '" style="vertical-align:middle;" /></a></p>';
		echo '<p>' . __( 'List of countries in this database', 'wpGeonames' ) . ' : <span style="font-weight:700;font-size:11px;">' . $cc . '</span></p>';
		?>
        ...
        <form method="post" id="wpGeonames_options3" name="wpGeonames_options3"
            ...
            <table class="form-table">
                <tr style="vertical-align: top;">
                    <th scope="row"><label
                                for="wpGeonamesPostalAdd"><?php _e( 'Add data to WordPress', 'wpGeonames' ); ?></label>
                    </th>
                    <td>
						<?php if ( ! empty( $wpGeoList['filenames']['postal'] ) ) { ?>
                            <select name="wpGeonamesPostalAdd" id="wpGeonamesPostalAdd" multiple="multiple">
								<?php foreach ( $wpGeoList['filenames']['postal'] as $country ) {
                ...
                <tr style="vertical-align: top;">
                    ...
                    <td style="width:250px;">
                        ...
                        <label>
                            ...
                            <span style="color:#bb2;">
                            <?php _e( 'Postal Code', 'wpGeonames' ); ?></span></label><br>
                        <label>
                        ...
                        <label>
							...
							<?php _e( 'Admin1 Name', 'wpGeonames' ); ?></label><br>
                    </td>
                    <td>
                        <label>
                    ...
                    <td>
                        ...
                        <label>
							...
							<?php _e( 'Accuracy', 'wpGeonames' ); ?></label><br>
                    </td>
                </tr>
            </table>
        ...
        <script type="text/javascript"
                src="<?php echo plugins_url(); ?>/wp-geonames/sumoselect/jquery.sumoselect.min.js"></script>
        <script type="text/javascript">
            jQuery(document).ready(function () {
	...
	function clear( $table ) {
		...
		if ( $table === self::tblLocations ) {
			...
			if ( empty( $a ) ) {
				$wpdb->query( "ALTER TABLE " . $this->tblLocations . " ADD INDEX `index1` ( `feature_class`,`feature_code`( 3 ),`country_code`,`cc2`( 2 ),`name`( 3 ))" );
			}
			// *********************************************
		}
	...
	function check_options( $options ) {
		...
		if ( empty( $options )
		     ...
		     || time() - ( $options['filenames']['lastUpdate'] ?: 0 ) > 60 * 60 * 24
		) {
	...
	function shortcode( $a ) {
		...
		} else if ( file_exists( get_stylesheet_directory() . '/templates/wp-geonames_location_taxonomy.php' ) ) {
...
//
	...
	function ajax_wpGeonamesAddLocation() {
		// AJAX Admin
		if ( $this->verifyToka() ) {
			return;
		}
	...
	function addLocationsFromForm( $mode, $url, $f ) {
		$fe = [];
...
// ********* OTHER *********
	...
	function ajax_get_city_by_country_region() {
		// AJAX Admin
